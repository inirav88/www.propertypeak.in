...
@section('title', 'Edit Project - Developer Dashboard')
...
<div class="container-fluid py-4">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form action="{{ route('developer-portal.projects.update', $project->slug) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                            ...
                            <option value="mixed_use" {{ old('type', $project->type) === 'mixed_use' ? 'selected' : '' }}>Mixed Use</option>
                            <option value="industrial" {{ old('type', $project->type) === 'industrial' ? 'selected' : '' }}>Industrial</option>
                            <option value="retail" {{ old('type', $project->type) === 'retail' ? 'selected' : '' }}>Retail</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="address" class="form-control @error('address') is-invalid @enderror" rows="2" required>{{ old('address', $project->address) }}</textarea>
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="date" name="launch_date" class="form-control @error('launch_date') is-invalid @enderror" 
                            value="{{ old('launch_date', $project->launch_date ? $project->launch_date->format('Y-m-d') : '') }}">
                        @error('launch_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="date" name="possession_date" class="form-control @error('possession_date') is-invalid @enderror" 
                            value="{{ old('possession_date', $project->possession_date ? $project->possession_date->format('Y-m-d') : '') }}">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="total_units" class="form-control @error('total_units') is-invalid @enderror" 
                            value="{{ old('total_units', $project->total_units) }}" min="1">
                        @error('total_units')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" name="price_starting_from" class="form-control @error('price_starting_from') is-invalid @enderror" 
                            value="{{ old('price_starting_from', $project->price_starting_from) }}" min="0" step="0.01">
                ...
                <div class="mb-3">
                    ...
                    @if($project->brochure)
                        <div class="mb-2">
                            <a href="{{ asset('storage/' . $project->brochure) }}" target="_blank" class="btn btn-sm btn-outline-info">
                                <i class="fas fa-file-pdf me-2"></i>View Current Brochure
                            </a>
                        </div>
                    @endif
                    <input type="file" name="brochure" class="form-control @error('brochure') is-invalid @enderror" 
                        accept=".pdf">
                    <small class="text-muted">Upload new brochure to replace. Max 10MB</small>
                    @error('brochure')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
