                    ...
                    add_filter(THEME_FRONT_HEADER, function ($html) use ($model) {
                        ...
                        if (get_class($model) == Property::class) {
                            $schema = [
                                ...
                                'floorSize' => [
                                    ...
                                    'value' => $model->square,
                                    'unitCode' => setting('real_estate_square_unit', 'm²'),
                                ],
                                'numberOfRooms' => $model->number_bedroom,
                                'numberOfBathroomsTotal' => $model->number_bathroom,
                                'price' => [
                            ...
                            if (RealEstateHelper::isEnabledZipCode() && $model->zip_code) {
                                $schema['address']['postalCode'] = $model->zip_code;
                            }
                        ...
                        if (get_class($model) == Project::class) {
                            ...
                            if ($model->price_from || $model->price_to) {
                                $schema['priceRange'] = [];
                                if ($model->price_from) {
                                    $schema['priceRange']['minPrice'] = $model->price_from;
                                }
                                if ($model->price_to) {
                                    $schema['priceRange']['maxPrice'] = $model->price_to;
                                }
                                $schema['priceRange']['priceCurrency'] = strtoupper(get_application_currency()->title);
                            }

                            if ($model->number_floor) {
                                $schema['floorLevel'] = $model->number_floor;
                            }
