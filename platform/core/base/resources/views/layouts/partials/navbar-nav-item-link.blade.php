...
<a @class([
    ...
    href="{{ $hasChildren ? "#$menu[id]-child" : $menu['url'] }}" id="{{ $menu['id'] }}" @if ($hasChildren)
        data-bs-toggle="{{ Str::startsWith($hasChildren ? "#$menu[id]-child" : $menu['url'], '#') ? 'collapse' : 'dropdown' }}"
        data-bs-auto-close="{{ $autoClose ?? 'false' }}" role="button"
    aria-expanded="{{ $menu['active'] ? 'true' : 'false' }}" @endif title="{{ $menu['title'] ?? $name }}">
