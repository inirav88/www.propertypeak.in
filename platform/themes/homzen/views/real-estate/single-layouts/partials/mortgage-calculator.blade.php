...
<div class="widget-box bg-surface header-property-detail mc-widget-v2" style="margin-top: 30px;">
    ...
    <div class="form-comment">
        ...
        <div class="button-submit" style="margin-top: 15px;">
            <button class="tf-btn primary mc-btn-calc" type="button">{{ __('Calculate') }}</button>
        </div>
    </div>
</div>
...
<script>
    (function () {
        ...
        document.body.addEventListener('click', function (e) {
            if (e.target && e.target.closest('.mc-btn-calc')) {
                e.preventDefault();
                var btn = e.target.closest('.mc-btn-calc');
                var container = btn.closest('.mc-widget-v2');
                runCalc(container);
            }
        });
        ...
        // Auto-run on load
        setTimeout(function () {
            var allWidgets = document.querySelectorAll('.mc-widget-v2');
            allWidgets.forEach(function (w) { runCalc(w); });
        }, 1000);
    })();
</script>