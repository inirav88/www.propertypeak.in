@php
    ...
    $selectedTabs = explode(',', $shortcode->tabs ?: 'project,rent,sale,pg');
    if (!in_array('pg', $selectedTabs)) {
        $selectedTabs[] = 'pg';
    }

    $defaultSearchType = $shortcode->default_search_type ?: 'project';
    $tabs = collect(['project' => __('Project'), 'rent' => __('For Rent'), 'sale' => __('For Sale'), 'pg' => __('PG')])
        ...
        ->reject(fn($tab, $key) => $key !== 'project' && $key !== 'pg' && !in_array($key, RealEstateHelper::enabledPropertyTypes()))
    ...
    <div class="flat-tab flat-tab-form">
            ...
            <ul @class(['nav-tab-form', 'style-1' => $style === 1, 'style-2' => in_array($style, [2, 3]), 'style-3' => $style === 4, 'justify-content-center' => $centeredTabs]) role="tablist">
                @foreach ($tabs as $key => $tab)
                    <li class="nav-tab-item" role="presentation">
                        <a href="javascript:void(0);" @class(['nav-link-item', 'active' => $key === $defaultSearchType])
                            data-bs-toggle="tab" data-bb-toggle="change-search-type" data-value="{{ $key }}"
        ...
        <div class="tab-content">
            <div class="tab-pane fade active show" role="tabpanel">
                <form @class(['form-sl', 'flat-filter-form' => $style === 3]) @if ($projectsSearchEnabled && $defaultSearchType === 'project') action="{{ RealEstateHelper::getProjectsListPageUrl() }}" @else
                action="{{ RealEstateHelper::getPropertiesListPageUrl() }}" @endif method="get" id="hero-search-form">

                    <input name="type" type="hidden" @if ($projectsSearchEnabled && $defaultSearchType === 'project')
                    value="project" @else value="{{ $defaultSearchType }}" @endif>
